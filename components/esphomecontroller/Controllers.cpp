...
:triggers(*(new Triggers()))
{
	_instance = this;
	//globlog += "CTOR";
	//DBG_OUTPUT_PORT.println("Controllers::Ctor");
...
void Controllers::loadconfig() {
	...
	if (SPIFFS.exists(filename)) {
		...
		if (configFile) {
			...
			if (!error) {
				//DBG_OUTPUT_PORT.println("Controllers::json No error");
...
void Controllers::setuphandlers(AsyncWebServer& server) {
	...
	DBG_OUTPUT_PORT.println("Controllers::setuphandlers");
	...
	for (int i = 0;i < this->GetSize();i++) {
		...
		String pathget = path + String("/get_state");
		server.on(pathget.c_str(), HTTP_AW_GET, [ctl](AsyncWebServerRequest *request) {
			DBG_OUTPUT_PORT.println("sending header from POST");
		...
		String pathset = path + String("/set_state");
		server.on(pathset.c_str(), HTTP_AW_OPTIONS, [] (AsyncWebServerRequest *request){
			//DBG_OUTPUT_PORT.println("start processing preflight");
		...
		server.on(pathset.c_str(), HTTP_AW_POST, [](AsyncWebServerRequest *request) {
			//DBG_OUTPUT_PORT.println("POST SET State");
