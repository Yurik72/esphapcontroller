...
server.on("/upload", HTTP_AW_POST, [](AsyncWebServerRequest *request) { request->send(200, "text/plain", ""); }, handleFileUpload); \
...
#endif
	DBG_OUTPUT_PORT.println("spiffs setup self done");
}
...
void handleFileBrowser(AsyncWebServerRequest *request) {
	DBG_OUTPUT_PORT.println("handleFileBrowser");
