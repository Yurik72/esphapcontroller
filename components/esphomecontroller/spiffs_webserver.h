...
#define SETUP_FILEHANDLES   server.on("/list", HTTP_AW_GET, handleFileList); \
  server.on("/edit", HTTP_AW_GET, []() { \
    if (!handleFileRead("/edit.htm")) server.send(404, "text/plain", "FileNotFound"); \
  }); \
  server.on("/edit", HTTP_AW_PUT, handleFileCreate); \
  server.on("/edit", HTTP_AW_DELETE, handleFileDelete); \
  server.on("/edit", HTTP_AW_POST, []() { \
    server.sendHeader("Access-Control-Allow-Origin", "*"); \
    server.send(200, "text/plain", ""); \
  }, handleFileUpload); \
  server.on("/jsonsave", handleJsonSave); \
  server.on("/jsonload", handleJsonLoad); \
  server.on("/upload", HTTP_AW_POST, []() { server.send(200, "text/plain", ""); }, handleFileUpload); \
...
void handleNotFound() {
	...
	message += server.uri();
	message += "\nMethod: ";
	message += (server.method() == HTTP_AW_GET) ? "GET" : "POST";
	message += "\nArguments: ";
	message += server.args();
	message += "\n";
